...
const allowedOrigins = (process.env.ALLOWED_ORIGINS || "https://v-shiroya-insurance.web.app,https://v-shiroya-policy.onrender.com,http://localhost:5173")
...
function parseJson(content: unknown): any | null {
  ...
  let text = content.trim().replace(/^```json\s*/i, "").replace(/^```\s*/i, "").replace(/\s*```$/i, "");
...
function loadPolicies(): any[] {
  ...
  } catch (error) { console.error("Policy DB read error", error); }
  return [];
}
...
async function analyzeDocument(fileData: string, fileName: string, mimeType: string, instruction: string) {
  ...
  const content: any[] = [{ type: "text", text: `You are an insurance policy document analyzer. Read the complete document carefully and return ONLY one valid JSON object. Extract policy number, insurer, owner/insured person, policy type, start date, end date, premium amount, sum assured/sum insured, nominee, phone, email, address, policy status, and all other useful details. Do not invent data. Instruction: ${instruction || "Analyze the complete policy document."}` }];
  if (isPdf) content.push({ type: "file", file: { filename: fileName || "policy.pdf", file_data: `data:application/pdf;base64,${clean}` } });
  else content.push({ type: "image_url", image_url: { url: `data:${mimeType || "image/jpeg"};base64,${clean}` } });
  ...
  for (const key of orderedKeys) {
    for (const model of models) {
      ...
      if (isPdf) payload.plugins = [{ id: "file-parser", pdf: { engine: pdfEngine } }];

      for (const withStructuredOutput of [true, false]) {
        const requestPayload = withStructuredOutput ? payload : (() => { const copy = { ...payload }; delete copy.response_format; return copy; })();
        try {
...
app.get("/api/health", (_req, res) => {
  res.json({ ok: true, service: "V-SHIROYA Policy AI", provider: "OpenRouter", keyCount: getKeys().length, models, pdfEngine, timestamp: new Date().toISOString() });
});
...
const analyzeHandler = async (req: express.Request, res: express.Response) => {
  try {
    ...
    const extraction = await analyzeDocument(fileData, fileName, mimeType || "application/pdf", instruction || "");
    return res.json({ success: true, extraction });
  } catch (error: any) {
    console.error("Policy analysis failed:", error?.message || error);
    return res.status(502).json({ success: false, error: "AI analysis failed.", details: error?.message || "Unknown backend error" });
  }
};
...
app.post("/api/policies", (req, res) => { const policies = loadPolicies(); const policy = { ...req.body, id: req.body?.id || `pol-${Date.now()}`, createdAt: new Date().toISOString() }; policies.unshift(policy); savePolicies(policies); res.json({ success: true, policy }); });
app.put("/api/policies/:id", (req, res) => { const policies = loadPolicies(); const i = policies.findIndex((p) => p.id === req.params.id); if (i < 0) return res.status(404).json({ error: "Policy not found" }); policies[i] = { ...policies[i], ...req.body, updatedAt: new Date().toISOString() }; savePolicies(policies); res.json({ success: true, policy: policies[i] }); });
app.delete("/api/policies/:id", (req, res) => { const policies = loadPolicies(); savePolicies(policies.filter((p) => p.id !== req.params.id)); res.json({ success: true }); });
...
app.listen(PORT, "0.0.0.0", () => {
  ...
  console.log(`OpenRouter keys: ${getKeys().length}; models: ${models.join(", ")}`);
});
